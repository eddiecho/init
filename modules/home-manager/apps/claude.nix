...
in {
  ...
  config = lib.mkIf cfg.enable {
    # Managed here rather than by hand: this file is regenerated on every
    # `make`/`make home`, so edits belong in this module, not in
    # ~/.claude/CLAUDE.md directly (they'll be clobbered on the next switch).
    home.file.".claude/CLAUDE.md".text = ''
      # CLAUDE.md

      Global guidance for Claude Code across all of ${config.settings.fullName}'s projects.
