...
`static` has static assets. In particular it has my Neovim and Hyprland configurations, both as Lua. It all gets symlinked into `~/.config`. I think it's better than writing Lua in Nix, and the source of truth is always the local copy of this repo.

`tools` has random one-off scripts, including a bootstrap installer (which as of 12/01/2025 I still haven't tested).

`hosts/home` is for machines that don't get a full NixOS/nix-darwin config from this flake. See `hosts/home/README.md`.
